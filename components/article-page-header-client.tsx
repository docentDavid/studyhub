"use client";

import Link from "next/link";
import { useLanguage } from "@/lib/use-language";
import { commonCopy } from "@/lib/i18n/copy";

export function ArticlePageHeaderClient() {
  const language = useLanguage();
  const common = commonCopy[language];

  return (
    <nav
      aria-label={common.portalName}
      className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 pt-16 sm:px-6 lg:px-8"
    >
      <Link
        href="/"
        className="inline-flex rounded-full border border-[var(--border)] bg-[var(--surface)] px-4 py-2 text-sm font-bold text-[var(--brand)] transition hover:bg-[var(--brand-soft)] focus:outline-none focus:ring-2 focus:ring-[var(--brand)]"
      >
        ← {common.goToHomepage}
      </Link>

      <Link
        href="/submit"
        className="inline-flex items-center rounded-full bg-[var(--brand-dark)] px-4 py-2 text-sm font-bold text-white transition hover:opacity-90 focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--brand)]"
      >
        {common.submitResource}
      </Link>
    </nav>
  );
}
